...
export class InMemoryMedicalRecordAllergiesRepository implements MedicalRecordAllergiesRepository {
  ...
  async createMany(medicalRecordAllergies: MedicalRecordAllergy[]): Promise<void> {
    this.items.push(...medicalRecordAllergies);
  }

  async deleteMany(medicalRecordAllergies: MedicalRecordAllergy[]): Promise<void> {
    medicalRecordAllergies.forEach(medicalRecordAllergy => {
      const medicalRecordAllergyIndex = this.items
        .findIndex(item => item.allergyId.toString() === medicalRecordAllergy.allergyId.toString() && item.medicalRecordId.toString() === medicalRecordAllergy.medicalRecordId.toString());
  ...
  async findManyByMedicalRecordId(medicalRecordId: string): Promise<MedicalRecordAllergy[]> {
  ...
  async deleteManyByAllergyId(allergyId: string): Promise<void> {
    this.items = this.items.filter(item => item.allergyId.toString() !== allergyId);
  }
