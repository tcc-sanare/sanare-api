...
export class InMemoryMedicalRecordChronicDiseasesRepository
  implements MedicalRecordChronicDiseasesRepository
{
  ...
  async createMany(
    medicalRecordChronicDiseases: MedicalRecordChronicDisease[],
  ): Promise<void> {
  ...
  async deleteMany(
    ...
    medicalRecordChronicDiseases.forEach(
      (medicalRecordChronicMedicalRecordChronicDisease) => {
        const medicalRecordChronicMedicalRecordChronicDiseaseIndex =
          this.items.findIndex(
            (item) =>
              item.chronicDiseaseId.toString() ===
                medicalRecordChronicMedicalRecordChronicDisease.chronicDiseaseId.toString() &&
              item.medicalRecordId.toString() ===
                medicalRecordChronicMedicalRecordChronicDisease.medicalRecordId.toString(),
          );
        ...
        this.items.splice(
          medicalRecordChronicMedicalRecordChronicDiseaseIndex,
          1,
        );
      },
    );
  ...
  ): Promise<MedicalRecordChronicDisease[]> {
  ...
  async deleteManyByChronicDiseaseId(chronicDiseaseId: string): Promise<void> {
    this.items = this.items.filter(
      (item) => item.chronicDiseaseId.toString() !== chronicDiseaseId,
    );
  }
}
